...
it('can update all intended properties on tab-items', async () => {
  ...
  let res = await client.put({ tabSet: 'new-tab-set' }, `/tab-items/${tabItem.id}`, defaultHeaders);
  expect(res.body.tabSet).toBe('new-tab-set');

  res = await client.put({ quantity: 99.99 }, `/tab-items/${tabItem.id}`, defaultHeaders);
  expect(res.body.quantity).toBe(99.99);

  res = await client.put({ remarks: 'new-remarks' }, `/tab-items/${tabItem.id}`, defaultHeaders);
  expect(res.body.remarks).toBe('new-remarks');

  res = await client.put({ street: 'new-street' }, `/tab-items/${tabItem.id}`, defaultHeaders);
  expect(res.body.street).toBe('new-street');

  res = await client.put({ side: 'new-side' }, `/tab-items/${tabItem.id}`, defaultHeaders);
  expect(res.body.side).toBe('new-side');

  res = await client.put({ beginStation: 5432 }, `/tab-items/${tabItem.id}`, defaultHeaders);
  expect(res.body.beginStation).toBe(5432);

  res = await client.put({ endStation: 12345 }, `/tab-items/${tabItem.id}`, defaultHeaders);
...
it('cannot update tab-items from other organizations', async () => {
